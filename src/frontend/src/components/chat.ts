...
export const defaultOptions: ChatComponentOptions = {
  ...
  promptSuggestions: [
    "Where can I see Gorrilas and Giraffes?",
    "Where's the collest place to take a mud bath in Africa?",
    "Are lions friendly?",
  ],
  messages: [],
  strings: {
    promptSuggestionsTitle: "Ask me about Africa Tourist Locations",
    ...
    chatInputPlaceholder: "Ask me about places to visit in Africa...",
    chatInputButtonLabel: "Send question",
    assistant: "African Tourism Expert",
    user: "You",
...
export class ChatComponent extends LitElement {
        ...
        for await (const chunk of chunks) {
          if (chunk.choices[0]?.delta.context?.data_points) {
            message.context!.data_points =
              chunk.choices[0].delta.context?.data_points;
            message.context!.thoughts =
              chunk.choices[0].delta.context?.thoughts ?? "";
      ...
      <div class="message assistant error">
        <div class="message-body">
          <span class="error-message"
            >${this.options.strings.errorMessage}</span
          >
  ...
  protected renderCitation = (citation: string, index: number) => {
    return html`<button
      class="citation"
      @click=${() => this.onCitationClicked(citation)}
    >
      ${index + 1}. ${citation}
    </button>`;
  };
  ...
  protected override render() {
    const parsedMessages = this.messages.map((message) =>
      parseMessageIntoHtml(message, this.renderCitationLink)
    );
    return html`
